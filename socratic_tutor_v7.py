...
def get_ai_feedback(model, concept, question, user_answer, discipline, history_context_string):
    ...
    # --- Discipline Guidance ---
    if discipline == "Mathematics": discipline_guidance = "..."
    elif discipline == "Engineering": discipline_guidance = "..."
    elif discipline == "Physics": discipline_guidance = "..."
    else: discipline_guidance = "..."
...
def get_ai_proof(model, concept, question, discipline, history_context_string):
    """Provides a formal proof-like explanation of the concept, using the pre-formatted history context string."""
    # --- Discipline Guidance ---
    if discipline == "Mathematics": discipline_guidance = "..."
    elif discipline == "Engineering": discipline_guidance = "..."
    elif discipline == "Physics": discipline_guidance = "..."
    ...
    prompt = f"""
    Context:
    You are an AI Economics Tutor providing a formal proof-like explanation of the main concept to a {discipline} student. Use the conversation history to tailor the explanation.
    ...
    Task:
    Provide a formal, structured explanation (8-12 sentences total) of the main economic concept, formatted with the following sections:
    - ASSUMPTIONS: List all hidden and explicit assumptions underlying the concept, including any relevant assumptions from the conversation history (e.g., student misconceptions or tutor clarifications). Tailor to {discipline} perspective.
    - THEORIES: List and concisely describe the major economic theories directly connected to the concept (1-2 sentences per theory).
    - REASONING STEPS: Provide a clear, step-by-step logical sequence starting from the assumptions, applying the theories, and leading to the main conclusion or answer related to the concept. Use concrete examples with numbers where applicable.
    - LIMITATIONS: Enumerate the major limitations and exceptions that may arise in this reasoning chain, including any discipline-specific challenges.
    - Ensure the explanation is rigorous yet accessible, using {discipline} terminology where appropriate to draw parallels or contrasts.
    - Include at least one concrete example with specific numbers/values.
    {discipline_guidance}

    Socratic Question for Context: "{question}"

    Your Formal Proof-Like Explanation (as Economics Tutor, tailored for {discipline} student):
    """
    try:
        safety_settings = [ {"category": c, "threshold": "BLOCK_MEDIUM_AND_ABOVE"} for c in ["HARM_CATEGORY_HARASSMENT", "HARM_CATEGORY_HATE_SPEECH", "HARM_CATEGORY_SEXUALLY_EXPLICIT", "HARM_CATEGORY_DANGEROUS_CONTENT"]]
        response = model.generate_content(prompt, safety_settings=safety_settings, generation_config=genai.types.GenerationConfig(temperature=0.5))
        if not response._result.candidates: reason = response.prompt_feedback.block_reason if response.prompt_feedback else "Unknown"; return f"[AI Proof Blocked: {reason}] Could not generate proof."
        if not response.parts or not hasattr(response.parts[0], 'text'): return "[AI Proof Unavailable: No text mistakes.]"
        return response.text.strip()
    except exceptions.GoogleAPIError as e: print(f"\nError during proof: {e}", file=sys.stderr); return "[AI Proof Error: API error.]"
    except Exception as e: print(f"\nUnexpected error during proof: {e}", file=sys.stderr); return "[AI Proof Error: Unexpected issue.]"


# --- Main Tutor Logic ---

def run_tutor():
    """Runs the main Socratic tutoring session with the new history strategy and proof option."""
        ...
        for j, question_text in enumerate(questions):
            ...
            while True: # Inner loop for user answer/command
                 try:
                     ...
                     user_input = input("Enter your answer (or enter one of following commands: 'hint', 'proof', 'menu', 'quit') ").strip()
                     lower_input = user_input.lower()

                     if lower_input == 'quit': sys.exit(print("\nExiting. Goodbye!"))
                     if lower_input == 'menu': print("\nReturning to menu..."); return_to_menu = True; break
                     if lower_input == 'hint':
                          ...
                          continue
                     if lower_input == 'proof':
                          print("Generating formal proof-like explanation...")
                          proof = get_ai_proof(model, selected_concept, question_text, discipline, history_context_string)
                          print(f"\nAI Tutor Proof:\n{proof}\n")
                          chat_history.append({"role": "user", "content": "(Requested a proof)"})
                          chat_history.append({"role": "assistant", "content": proof})
                          continue
                     ...
                     # *** Manage history AGAIN before getting feedback ***
                     history_context_string, current_summary = manage_history_and_get_context(
                         model, current_summary, chat_history, discipline
                     )
